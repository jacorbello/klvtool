...
Each `*.hex` file contains one raw UAS Datalink LS packet, hex-encoded, whitespace allowed. Tests in `internal/klv/corpus_test.go` decode each file and snapshot the JSON output at the adjacent `*.json` path.
